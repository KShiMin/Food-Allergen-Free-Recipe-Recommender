...
def dashboard():
    ...
    # 2) Caloric budget logic
    remaining_calories = get_remaining_calories(user)

    # 3) Get start of week and fetch logs
    ...
    # 4) Enrich logs from MongoDB + timestamp parse
    mongo = MongoCRUD()
    saved = []
    for recipe_id, cals, ts in logs:
        ...
        if isinstance(ts, str):
            try:
                ...
                print("❌ Could not parse timestamp:", ts)
                ts = None
...
def save_recipe(recipe_id):
    ...
    # 1) Insert the log
    try:
        print("🔍 Logging meal:", user_id, recipe_id, calories)
        cur.execute("""
            INSERT INTO meal_log (user_id, recipe_id, calories, timestamp)
            VALUES (?, ?, ?, ?)
        """, (user_id, int(recipe_id), calories, datetime.now()))
        conn.commit()
        print("✅ Inserted meal_log entry!")
    except Exception as e:
        print("❌ Failed to insert into meal_log:", e)
